...
<v-ons-page v-if="accountData != ''">
  <custom-toolbar backLabel="Back" :title="'Edit'">
    <template slot="right">
    ...
    <v-ons-list>
      ...
      <v-ons-list-item>
        <b-field style="width: 100%">
          <b-select v-model="accountData.projectId" expanded placeholder="Select ..">
            <option v-for="projectName in projectNames" :key="projectName.id" :value="projectName.id">
              {{ projectName.name }}
            </option>
          </b-select>
...
export default {
  ...
  created() {
    this.getCashTitleIncome(), this.getCashTitleExpense();
    this.getProjectName();
  },
  computed: {
  ...
  methods: {
    ...
    editIncome: function() {
      ...
      accountRef.update({
        ...
        projectId: this.accountData.projectId, // ProjectID
        queryDay: this.editDate.date.getDate(),
        monthId:
          this.editDate.date.getFullYear() + "-" + this.editDate.date.getMonth()
      });
